...
# encodes a character it's numerical form
...
# LF mapped to 127-30
# decodes a character form the number form to it;s original character
...
# encodes a text to a numerical sequence
def encode_text(s):
    """Encode a string.
...
# decodes an encoded text to it's original form, eventually replacing Tabs and new lines with \
...
# gives one random character from the topn most probably to appear from the ALPHABET
...
# divides the training input data into minibaches that are run form epoch to epoch
