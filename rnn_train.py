...
# load data
dir = "C:/Users/achiriac/Desktop/relevant_separate_data/*.txt"
codetext, valitext, bookranges = txt.read_data_files(dir, validation=True)
...
# input state
with tf.name_scope('hidden_state') as scope:
...
# stats for display
with tf.name_scope('display_data') as scope:
    seqloss = tf.reduce_mean(loss, 1)
    batchloss = tf.reduce_mean(seqloss, name="batch_loss")
    accuracy = tf.reduce_mean(tf.cast(tf.equal(Y_, tf.cast(Y, tf.uint8)), tf.float32), name="accuracy")
...
with tf.device("/device:GPU:0") as device:
    for x, y_, epoch in txt.rnn_minibatch_sequencer(codetext, BATCHSIZE, SEQLEN, nb_epochs=200):
